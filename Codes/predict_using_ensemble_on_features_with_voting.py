...
def findConditionalProbForLM(lmIndices, trainLabels):
    ...
    for index in indexWiseLabelCount:
        ...
        for label in indexWiseLabelCount[index]:
            tempDict[label] = indexWiseLabelCount[index][label] / \
                sum(indexWiseLabelCount[index].values())
...
def findPredictionsFromVoting(a1, a2, a3):
    assert a1.shape == a2.shape == a3.shape
    a1, a2, a3 = a1.tolist(), a2.tolist(), a3.tolist()
    finalPredictions = list()
    for i in range(len(a1)):
        predictions = [a1[i], a2[i], a3[i]]
        countPredictions = list(
            map(lambda x: predictions.count(x), predictions))
        dictPredWithCount = dict(zip(predictions, countPredictions))
        predSortedWithCount = sorted(
            dictPredWithCount, key=dictPredWithCount.get)
        finalPredictions.append(predSortedWithCount[-1])
...
def main():
    ...
    classifier3File = argv[4]
    classifier1 = loadObjectFromFile(classifier1File)
    classifier2 = loadObjectFromFile(classifier2File)
    classifier3 = loadObjectFromFile(classifier3File)
    wordTfIdfVect = loadObjectFromFile(argv[5])
    charTfIdfVect = loadObjectFromFile(argv[6])
    predFile = argv[7]
    typeOfFile = int(argv[8])
    dialectFile = argv[9]
    testLMFile = argv[10]
    coarseProbFile = argv[11]
    ...
    testLMScoresArrayMax = testLMScoresArrayMax.reshape(
        (testLMScoresArrayMax.shape[0], 1))
    ...
    testCoarseProbIndices = testCoarseProbsMax.reshape(
        (testCoarseProbsMax.shape[0], 1))
    # # testCoarseProbIndices = testCoarseProbsMax.reshape((testCoarseProbsMax.shape[0],)).tolist()
    oneHotTestCoarseProbsMax = convertIntoOneHot(
        testCoarseProbIndices, len(dialects))
    ...
    testLMSIndicesOneHot = convertIntoOneHot(
        testLMScoresArrayMax, len(dialects))
    ...
    combinedTestTfIdf = hstack(
        [wordTestTfIdf, charTestTfIdf, testLMSIndicesOneHot, oneHotTestCoarseProbsMax])
    ...
    testPredictions1 = classifier1.predict(combinedTestTfIdf)
    testPredictions2 = classifier2.predict(combinedTestTfIdf)
    testPredictions3 = classifier3.predict(combinedTestTfIdf)
    # avgTestPredictions = np.argmax(0.2 * testPredictionProb1 + 0.8 * testPredictionProb2, axis=1)
    finalTestPredictions = findPredictionsFromVoting(
        testPredictions1, testPredictions2, testPredictions3)
