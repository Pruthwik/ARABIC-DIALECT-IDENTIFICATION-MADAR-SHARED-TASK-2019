...
def main():
    ...
    trainLMIndicesMax = lmScoresArrayMax.reshape((lmScoresArrayMax.shape[0], 1))
    # trainLMIndicesMax = lmScoresArrayMax.reshape((lmScoresArrayMax.shape[0],)).tolist()
    # print(trainLMIndicesMax.shape)
    coarseProbs = np.load(coarseProbFile)
    coarseProbsMax = np.argmax(coarseProbs, axis=1)
    trainCoarseProbsMax = coarseProbsMax.reshape((coarseProbsMax.shape[0], 1))
    # trainCoarseProbsMax = coarseProbsMax.reshape((coarseProbsMax.shape[0],)).tolist()
    oneHotCoarseProbsMax = convertIntoOneHot(trainCoarseProbsMax, len(dialects))
    ...
    # trainLMIndexWiseLabels = findConditionalProbForLM(trainLMIndicesMax, trainLabels)
    # trainCoareseProbIndexWiseLabels = findConditionalProbForLM(trainCoarseProbsMax, trainLabels)
    # lmScoresMatrix = coo_matrix(lmScoresArray)
    oneHotLMScoresMaxMatrix = convertIntoOneHot(trainLMIndicesMax, len(dialects))
    ...
    # combinedTrainTfIdf = hstack([wordTrainTFIdf, charTrainTFIdf, trainLMIndicesMax, trainCoarseProbsMax])
    # combinedTrainTfIdf = hstack([wordTrainTFIdf, charTrainTFIdf])
    combinedTrainTfIdf = hstack(
        [wordTrainTFIdf, charTrainTFIdf, oneHotLMScoresMaxMatrix, oneHotCoarseProbsMax])
    # combinedTrainTfIdf = hstack([wordTrainTFIdf, charTrainTFIdf, oneHotLMScoresMaxMatrix])
    ...
    # dumpObjectIntoFile('train-lmIndex-wise-labels-' + classifier + '.pkl', trainLMIndexWiseLabels)
    # dumpObjectIntoFile('train-coarse-index-wise-labels-' + classifier + '.pkl', trainCoareseProbIndexWiseLabels)
