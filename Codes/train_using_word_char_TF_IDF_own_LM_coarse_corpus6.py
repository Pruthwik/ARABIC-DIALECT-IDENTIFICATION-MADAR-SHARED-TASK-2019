...
def main():
    ...
    dumpObjectIntoFile('train-vect-pandas-with-combined-LM-max-one-hot-corpus-6-' + word_analyzer + '-' +
                       '-'.join(map(str, word_ngram_range)) + '-' + classifier + '.pkl', wordTfIdfVect)
    dumpObjectIntoFile('train-vect-pandas-with-combined-LM-max-one-hot-corpus-6-' + char_analyzer + '-' +
    ...
    dumpObjectIntoFile(classifier + '-' + word_analyzer + '-' +
                       ...
                       '-'.join(map(str, char_ngram_range)) + '-pandas-with-combined-LM-max-one-hot-corpus-6.pkl', classifierToSelect)
    dumpObjectIntoFile('train-lmIndex-wise-labels-' + classifier + '-corpus6.pkl', trainLMIndexWiseLabels)
